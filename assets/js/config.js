  ...
  SN.changelog = [
    {
      version: "v0.0.10",
      date: "2026-09-17",
      title: "桌面编辑模式 + 自由摆放 + 滑动换页 + 角色头像图片",
      items: [
        "桌面支持长按图标进入编辑模式（模仿 iOS：图标抖动）",
        "编辑模式下图标可放在任意行列的固定空位，允许图标之间留空；落到已有图标上则交换位置，布局随备份保存",
        "拖到屏幕左右边缘自动翻页，没有页面时自动新建（最多 5 页）；空页在结束编辑时整理，不在拖动中突然跳页",
        "主页面支持从图标、小组件或空白处左右滑动：页面跟手移动，松手平滑翻页或回弹；点页点及拖图标跨页也有滑动动画",
        "修复：点进其他应用再退出来会自动进入编辑模式的问题（长按计时器没有清理）",
        "页点固定显示在 Dock 栏上方，不再随页面内容滚动",
        "角色编辑新增「头像图片」：选图自动压缩保存，聊天列表、角色卡、编辑页同步显示，随备份走",
        "修复：美化 → 自定义应用图标列表混进了隐藏页「角色编辑」，现在只列桌面和 Dock 上的应用",
        "设置 → 正则的列表里现在直接显示每条规则的用途说明"
      ]
    },
    {
      version: "v0.0.9",
      ...
      items: [
        "角色集新增「新建角色卡」：起名、写人设和开场白、挑头像配色，立即可聊",
        "角色编辑页新增「删除这个角色」（官方角色不可删），删除会连同聊天记录一起清掉",
        "世界书新增「新建条目」：标题、分类、触发关键词、内容、常驻开关",
        "世界书条目可以删除；命中逻辑不变——聊到关键词自动进提示词，常驻则每次都带",
        "自定义角色与世界书条目都会随备份文件一起导出/导入，恢复出厂时清空（官方角色自动保留）"
      ]
    },
    {
      version: "v0.0.8",
      ...
      items: [
        "提示词管线打通：预设 → 角色设定 → 命中的世界书 → 命中的记忆库 → 最近上下文 → 调 API → 跑正则 → 按 ||| 分气泡",
        "设置新增「提示词预设」：新建 / 编辑预设（支持 {{char}} {{user}} 占位符），可一键设为当前使用",
        "设置新增「正则」：给 AI 回复自动查找替换，可选整段跑或每个气泡单独跑，内置 4 条常用规则（默认 3 条开启）",
        "设置新增「记忆库」：随手记小事，配关键词命中或常驻，聊到相关话题会自动带进提示词",
        "AI 回复里的 ||| 会自动拆成多个气泡；用户消息不受影响",
        "代码重构为四层：界面层（ui/）、逻辑层（logic/）、数据层（data/）、设置层，对外行为不变"
      ]
    },
    {
      version: "v0.0.7",
    ...
    },
    {
      ...
      title: "页头：返回键与大标题合体",
      items: [
        "每个页面的大标题移到页面左上角，与退出键连成一体（箭头 + 大标题是一整块按钮）",
        "退出键只保留一个箭头：去掉「桌面 / 返回」文字，去掉胶囊磨砂玻璃底",
        "箭头用强调蓝、大标题用正文色；标题过长时自动省略号，不再换行撑高页头"
      ]
    },
    {
      version: "v0.0.5",
